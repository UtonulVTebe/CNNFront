...
public partial class BlankViewerViewModel(BlankTemplateService templateService) : ObservableObject
{
    ...
    /// <summary>По умолчанию загружает локальный путь или HTTP без авторизации; ученик может переопределить.</summary>
    protected virtual void OnPageImageRequired(BlankPageDefinition page) => LoadPageImage(page.ImagePath);
